...
*/
	...
	{
		if(preg_match('/^[a-zA-Z0-9_]{3,24}$/', $team_name)) {
			return true;
		} else {
	...
	{
		// TODO: Shouldn't this use country names map?
	...
	function validate_language($programming_langugage, &$err_msg)
	{
		if(preg_match('/^[a-zA-Z0-9_+]{1,16}$/', $programming_langugage)) {
			return true;
		} else {
			$err_msg = 'Invalid programming language.';
			return false;
		}
	}
	...
	function validate_lib_filename($file, &$err_msg)
	{
		...
		global $max_file_size_kb;
		$pass_name  = $file['name']  == $submission_file;
		$pass_err   = $file['error'] == 0;
		$pass_size  = $file['size'] < ($max_file_size_kb*1024);//max file size is 300MB

		if(!$pass_err)
			$err_msg = 'Error while uploading the file!';
		if(!$pass_name)
			$err_msg = 'The file name should be <strong>'.$submission_file.'</strong>.';
		if(!$pass_size)
			$err_msg = 'The file is too big! Max size is '.$max_file_size_kb.'KB.';
	...
	{
		global $db_salt;
		return openssl_encrypt($text, 'AES-128-CBC', $db_salt);
	}

	function simple_decrypt($text)
	{
		global $db_salt;
		return openssl_decrypt($text, 'AES-128-CBC', $db_salt);
	}
